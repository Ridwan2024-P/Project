...
if ($_SERVER["REQUEST_METHOD"] === "POST") 
{
    ...
    if ($result['success']) {
        $message = "<p style='color:green;text-align:center;'>Evaluation submitted successfully! 
    ...
    <tr>
        ...
        <td>
            <input type='radio' name='{$name}_select' onclick=\"
                ...
                document.getElementById('{$dev_id}').required = true;
                document.getElementById('{$acc_id}').required = false;
        ...
        <td>
            <input type='radio' name='{$name}_select' onclick=\"
                ...
                document.getElementById('{$acc_id}').required = true;
                document.getElementById('{$dev_id}').required = false;
...
if ($eval_result && $eval_result->num_rows > 0) {
    ...
    while ($row = $eval_result->fetch_assoc()) {
        ...
        $avg_res = $db->getConnection()->query(
            "SELECT avg_score FROM group_averages WHERE group_number='{$row['group_number']}'"
        );

        $avg_val = ($avg_res && $avg_res->num_rows > 0) 
                   ? $avg_res->fetch_assoc()['avg_score'] 
                   : '-';
